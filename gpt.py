...
def save_tax(content):
    with open("generated_rules/taxonomy.xml", "w", encoding="utf8") as tax:
        tax.write(content)


def get_taxonomy(domain, minimum_elements=70):
...
Give me at least {str(minimum_elements)} elements
    """}
        ...
        content = completion.choices[0].message.content
        if content.startswith("<DOMAINTREE>") and content.endswith("</DOMAINTREE>"):
